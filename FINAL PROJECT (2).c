#include<stdio.h>
#include<string.h>                            //ROLL NUMBERS OF GROUP MEMBERS//
#include<stdlib.h>       //SHAH HUSSAIN : 19K-1446   MUHAMMAD FAIZ : 19K-0247    SARFARAZ ALI : 19K-1281//
#include<conio.h>

#include "resturant.h"//INCLUDING HEADER FILE OF RESTURANT
...
void room_reservation(void)
{
  ...
  int total,grandtotal,discount=0;
	...
	scanf("%d",&ptr->days);
	fprintf(fp,"\n%s\n%s\n%d\n%d\n%d\n%d\n%d",ptr->name,ptr->address,ptr->dd,ptr->mm,ptr->yy,ptr->phone,ptr->nic);
	fclose(fp);
	char roomtype[30];
...
void main_menu(void)
{
	system("color 9");
       ...
       scanf("%d",&ch);
       switch(ch)
    {//NOW CALLING FUNCTIONS DEFINED ABOVE//
     case 1:
     	{
		 room_reservation();
		}
	 ...
	 {
	 	system("color 4");
        resturant();//CALLING FUNCTION FROM HEADER FILE
	 }
     break;
